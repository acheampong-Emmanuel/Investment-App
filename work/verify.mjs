...
const checks = {
  ...
  backendRoutes: ["/api/health", "/api/countries", "/api/market-overview", "/api/search", "/api/quote", "/api/history", "/api/fundamentals", "/api/brokers", "/api/watchlist/refresh", "/api/compare", "/api/news", "/api/ai-insight", "/api/huggingface/connect", "/api/huggingface/test", "/api/huggingface/disconnect", "/api/ai/analyze", "/api/ai/recommend", "/api/ai/risk-score", "/api/ai/summarize-market", "/api/diagnostics"].every((route) => server.includes(route)),
  ...
  huggingFaceApi: server.includes("HF_ROUTER_BASE_URL") && server.includes("callHuggingFaceChat") && server.includes("encryptToken") && app.includes("connectHuggingFace") && app.includes("getHuggingFaceStatus") && html.includes("hfConnectToken") && html.includes("Hugging Face Connection") && !app.includes("router.huggingface.co/v1/chat/completions"),
